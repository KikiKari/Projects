#!/usr/bin/env tclsh
# 3d_166113.py — portiert nach tcl
# Quelle: python, Projects@abstractions:python/3d_166113.py
...
# Generiert die 3D-Architektur-Visualisierungs-HTML-Datei für TikTok LIVE Companion Android.
...
proc main {argv} {
    ...
    if {[llength $argv] != 1} {
        puts "Usage: [info script] <output_file>"
        exit 1
    }
    ...
    # Write to file
    if {[catch {set f [open $output_file w]} err]} {
        puts "Error opening file $output_file: $err"
        exit 1
    }
    
    if {[catch {puts -nonewline $f $html_content} err]} {
        puts "Error writing to file $output_file: $err"
        close $f
        exit 1
    }
    
    if {[catch {close $f} err]} {
        puts "Error closing file $output_file: $err"
        exit 1
    }
