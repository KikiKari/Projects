...
proc generate_periodic_table {filename} {
    ...
    puts $fh $html
    close $fh
    puts "Periodic table written to $filename"
}

proc generate_html_content {} {
    set elements [get_elements_data]
    set categories [get_categories]
    set category_labels [get_category_labels]
    
    set html {}
    append html [generate_doctype]
    append html [generate_html_head]
    append html "<body>\n"
    append html "<h1>Periodensystem der Elemente</h1>\n"
    append html "<p class=\"subtitle\">Klicke auf ein Element, um Details zu sehen.</p>\n"
    append html [generate_detail_section]
    append html [generate_main_grid $elements $categories]
    append html [generate_fblock_grid $elements $categories]
    append html [generate_legend $category_labels]
    append html [generate_javascript $elements $categories $category_labels]
    append html "</body>\n"
    append html "</html>\n"
...
proc generate_detail_section {} {
    return "<div class=\"detail\" id=\"detail\"><div class=\"detail-empty\">Klicke auf ein Element, um Details zu sehen</div></div>\n"
}

proc generate_main_grid {elements categories} {
    set html "<div class=\"grid\" id=\"main\">\n"
    
    # Add f-block markers
    foreach {r cat txt} {6 la "57-71" 7 ac "89-103"} {
        append html "<div class=\"cell cell-marker cat-$cat\" style=\"grid-column:3; grid-row:$r\">$txt</div>\n"
    }
    
    # Add elements (excluding f-block)
    foreach el $elements {
        lassign $el z sym name mass cat row col
        if {$row < 8} {
            set period $row
            append html "<button class=\"cell cat-$cat\" style=\"grid-column:$col; grid-row:$period\" data-z=\"$z\" title=\"$name ($sym)\" onclick=\"showDetail([$z,'$sym','$name','$mass','$cat',$row,$col])\">\n"
            append html "  <span class=\"num\">$z</span><span class=\"sym\">$sym</span>\n"
            append html "</button>\n"
        }
    }
...
proc generate_fblock_grid {elements categories} {
    set html "<div class=\"grid fgrid\" id=\"fblock\">\n"
    
    # Add f-block elements
    foreach el $elements {
        lassign $el z sym name mass cat row col
        if {$row >= 8} {
            set grid_row [expr {$row - 7}]
            append html "<button class=\"cell cat-$cat\" style=\"grid-column:$col; grid-row:$grid_row\" data-z=\"$z\" title=\"$name ($sym)\" onclick=\"showDetail([$z,'$sym','$name','$mass','$cat',$row,$col])\">\n"
            append html "  <span class=\"num\">$z</span><span class=\"sym\">$sym</span>\n"
            append html "</button>\n"
        }
    }
...
proc generate_legend {category_labels} {
    set html "<div class=\"legend\" id=\"legend\">\n"
    
    foreach {key label} $category_labels {
        append html "  <div class=\"legend-item\">\n"
        append html "    <span class=\"legend-swatch cat-$key\"></span><span>$label</span>\n"
        append html "  </div>\n"
    }
...
proc generate_javascript {elements categories category_labels} {
    # Convert data to JavaScript format
    set js_elements {}
    foreach el $elements {
        lassign $el z sym name mass cat row col
        append js_elements "\[$z,'$sym','$name','$mass','$cat',$row,$col\],"
    }
    set js_elements [string trimright $js_elements ","]
    
    set js_categories {}
    foreach {key value} $categories {
        append js_categories "'$key':'$value',"
    }
    set js_categories [string trimright $js_categories ","]
    
    set js_category_labels {}
    foreach {key value} $category_labels {
        append js_category_labels "'$key':'$value',"
    }
    set js_category_labels [string trimright $js_category_labels ","]
    
    return "<script>
const E = [$js_elements];
const CAT = {$js_categories};
const CATPL = {$js_category_labels};
...
generate_periodic_table $output_file
