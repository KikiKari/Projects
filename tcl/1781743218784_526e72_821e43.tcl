...
# 1781743218784_526e72.py — portiert nach tcl
...
proc main {} {
    ...
    set options [list [list OutputPath.arg "" "The path where to save the HTML file"]]
    set usage ": $::argv0 OutputPath"
    ...
    if {![info exists OutputPath] || $OutputPath eq ""} {
        puts stderr "Error: OutputPath is required"
        exit 1
    }
...
expBtn.onclick=()=>{ if(!VAULT)return; result.value=JSON.stringify(VAULT,null,2); };
    ...
    if {[catch {set f [open $OutputPath w]} err]} {
        puts stderr "Error opening file for writing: $err"
        exit 1
    }
    ...
    if {[catch {close $f} err]} {
        puts stderr "Error closing file: $err"
        exit 1
    }
}
