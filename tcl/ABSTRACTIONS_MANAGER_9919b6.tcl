...
proc _setup_logger {} {
    ...
    set log_level [expr {[info exists ::env(ABSTRACTIONS_LOG_LEVEL)] ? [string toupper $::env(ABSTRACTIONS_LOG_LEVEL)] : "INFO"}]
    set log_level [expr {$log_level eq "DEBUG" ? 10 : $log_level eq "INFO" ? 20 : $log_level eq "WARNING" ? 30 : $log_level eq "ERROR" ? 40 : 20}]
...
proc log_message {level message} {
    global logger
    set level_num [expr {$level eq "DEBUG" ? 10 : $level eq "INFO" ? 20 : $level eq "WARNING" ? 30 : $level eq "ERROR" ? 40 : 20}]
    if {$level_num >= $logger} {
        set timestamp [clock format [clock seconds] -format "%Y-%m-%d %H:%M:%S"]
        puts "$timestamp | $level | [info level -1]:[info frame] | $message"
    }
}
...
proc load_state {} {
    ...
    if {[catch {set fh [open $STATE_FILE r]}]} {
        log_message ERROR "State-File konnte nicht gelesen werden ($STATE_FILE): $fh"
        return $default_state
    }
    
    if {[catch {set content [read $fh]}]} {
        log_message ERROR "State-File konnte nicht gelesen werden ($STATE_FILE): $content"
        close $fh
        return $default_state
    }
    ...
    if {[catch {set state [::json::json2dict $content]}]} {
        log_message ERROR "State-File konnte nicht geparst werden ($STATE_FILE): $state"
        return $default_state
    }
...
proc save_state {state} {
    ...
    set tmp_path [file join $state_dir ".abstractions_state_[pid].tmp"]
    
    if {[catch {set fh [open $tmp_path w]}]} {
        log_message ERROR "State konnte nicht gespeichert werden: $fh"
        return
    }
    
    if {[catch {puts $fh [::json::dict2json $state]}]} {
        log_message ERROR "State konnte nicht gespeichert werden: $fh"
        close $fh
    ...
    if {[catch {file rename -force $tmp_path $STATE_FILE}]} {
        log_message ERROR "State konnte nicht gespeichert werden: $tmp_path -> $STATE_FILE"
        file delete $tmp_path
        return
    }
...
proc check_node_status {node_id} {
    global NODES
    if {[catch {set result [exec openclaw nodes status $node_id]}]} {
...
proc get_node_by_priority {{job_weight "medium"}} {
    ...
    if {![info exists weight_to_preference($job_weight)]} {
        set preferred_order [list node1 node2]
    } else {
        set preferred_order $weight_to_preference($job_weight)
    }
...
proc find_scripts_in_dir {directory {exclude_patterns {node_modules .git __pycache__ dist build}}} {
    ...
    foreach glob_pattern {"*.py" "*.js" "*.sh" "*.pl" "*.rb"} {
        foreach script_path [glob -nocomplain -dir $directory -types f -r $glob_pattern] {
            set exclude_found 0
            foreach pattern $exclude_patterns {
                if {[string first $pattern $script_path] != -1} {
                    set exclude_found 1
                    break
                }
            }
...
proc _build_stub_content {script_path target_lang source_lang template} {
    ...
    if {[catch {set fh [open $script_path r]}]} {
        log_message WARNING "Originaldatei konnte nicht gelesen werden: $fh"
    } else {
        set content [read $fh]
        close $fh
        set lines [split $content "\n"]
        set original_lines [lrange $lines 0 14]
    }
    ...
    return [format "%s\n%s %s - %s Version\n%s Portiert von %s\n%s Original: %s\n%s Erstellt: %s\n\n%s\n%s Original-Code-Referenz:\n%s\n%s" \
        ...
        $comment_char [file rootname [file tail $script_path]] \
        [string totitle $target_lang] \
...
proc create_abstraction {script_path target_lang} {
    ...
    set source_lang [file extension $script_path]
    set source_lang [string trimleft $source_lang "."]
    set source_lang [expr {[info exists ext_map($source_lang)] ? $ext_map($source_lang) : [string totitle $source_lang]}]
    
    set target_dir [file join $ABSTRACTIONS_REPO $target_lang]
    if {[catch {file mkdir $target_dir}]} {
        log_message ERROR "Zielverzeichnis konnte nicht erstellt werden ($target_dir): $target_dir"
        return 0
    }
    ...
    if {[catch {set fh [open $tmp_path w]}]} {
        log_message ERROR "Stub konnte nicht geschrieben werden ($target_file): $fh"
        file delete $tmp_path
        return 0
    }
    
    if {[catch {puts -nonewline $fh $content}]} {
        log_message ERROR "Stub konnte nicht geschrieben werden ($target_file): $fh"
        close $fh
        file delete $tmp_path
        return 0
    ...
    if {[catch {file rename -force $tmp_path $target_file}]} {
        log_message ERROR "Stub konnte nicht geschrieben werden ($target_file): $tmp_path -> $target_file"
        file delete $tmp_path
        return 0
    }
...
proc git_commit {message} {
    ...
    if {[catch {exec git -C $repo_str add .} result]} {
        log_message WARNING "Git-Befehl fehlgeschlagen: $result"
        return
    }
    
    if {[catch {exec git -C $repo_str commit -m $message} result]} {
        log_message WARNING "Git-Befehl fehlgeschlagen: $result"
        return
    }
...
proc create_status_report {state} {
    ...
    foreach lang_dir [glob -nocomplain -dir $ABSTRACTIONS_REPO -types d *] {
        ...
        if {[info exists TARGET_LANGUAGES($lang_name)]} {
            set lang_counts($lang_name) [llength [glob -nocomplain -dir $lang_dir -types f *]]
        }
    }
...
proc main {} {
    ...
    log_message INFO "State geladen: [llength [dict get $state processed]] bereits verarbeitet"
    ...
    set total_abstractions 0
    foreach lang [array names ::TARGET_LANGUAGES] {
        set lang_dir [file join $::ABSTRACTIONS_REPO $lang]
        if {[file exists $lang_dir]} {
            incr total_abstractions [llength [glob -nocomplain -dir $lang_dir -types f *]]
        }
    }
    dict set state stats abstractions_created $total_abstractions
