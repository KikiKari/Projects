...
proc main {argv} {
    ...
    if {[catch {set fid [open $outputFile w]} errorMsg]} {
        puts stderr "Could not open file '$outputFile' for writing: $errorMsg"
        exit 1
    }
    
    if {[catch {puts $fid $htmlContent; close $fid} errorMsg]} {
        puts stderr "Could not write to file '$outputFile': $errorMsg"
        exit 1
    }
