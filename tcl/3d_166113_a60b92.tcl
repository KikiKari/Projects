#!/usr/bin/tclsh
# 3d_166113.pl — portiert nach tcl
...
array set SPEC {
    schichten {
        {name "Quelle" farbe "#5f6773" blocks {
        ...
        {name "Bruecke" farbe "#2481cc" blocks {
        ...
        {name "Audio" farbe "#b45309" blocks {
        ...
        {name "Ausgabe" farbe "#fe2c55" blocks {
...
proc to_json {data} {
    if {[dict exists $data]} {
        ...
        foreach {key value} $data {
            lappend pairs "\"$key\":[to_json $value]"
        }
        append json [join $pairs ","]
        append json "\}"
    } elseif {[llength $data] > 1 && [string is list $data]} {
        set json "\["
        set items [list]
        foreach item $data {
            lappend items [to_json $item]
        }
        append json [join $items ","]
        append json "\]"
    } elseif {$data eq "null" || $data eq "true" || $data eq "false"} {
        set json $data
        ...
        regsub -all {"} $escaped {\"} escaped
        regsub -all {\n} $escaped {\\n} escaped
        regsub -all {\r} $escaped {\\r} escaped
        regsub -all {\t} $escaped {\\t} escaped
        set json "\"$escaped\""
    }
    return $json
...
set json_spec [to_json [array get SPEC]]

# HTML-Datei generieren
set html_content [format $html_template $json_spec]

# In Datei schreiben
if {[catch {open $ausgabe_datei w} fh]} {
    puts stderr "Kann Datei '$ausgabe_datei' nicht öffnen: $fh"
    exit 1
}
