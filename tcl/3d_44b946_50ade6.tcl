...
# Generiert die Spezifikationsdatenstruktur für die 3D-Visualisierung
proc get_spec_data {} {
    ...
    # Erfassungsschicht
    set erfassung [dict create]
    dict set erfassung name "Erfassung"
    dict set erfassung farbe "#5f6773"
    set erfassung_blocks [list]
    lappend erfassung_blocks [dict create id "getusermedia" name "getUserMedia" untertitel "MediaStream"]
    lappend erfassung_blocks [dict create id "4k-auswahl" name "4K-Auswahl" untertitel "3840x2160"]
    lappend erfassung_blocks [dict create id "geraete-dropdown" name "Geraete-Dropdown" untertitel "mehrere Kameras"]
    dict set erfassung blocks $erfassung_blocks
    lappend schichten $erfassung
    
    # On-Device-KI-Schicht
    set ondevice [dict create]
    dict set ondevice name "On-Device-KI"
    dict set ondevice farbe "#2481cc"
    set ondevice_blocks [list]
    lappend ondevice_blocks [dict create id "tensorflow-js" name "TensorFlow.js" untertitel "im Browser"]
    lappend ondevice_blocks [dict create id "coco-ssd-v2" name "COCO-SSD v2" untertitel "80 Klassen"]
    lappend ondevice_blocks [dict create id "webgl" name "WebGL" untertitel "bald WebGPU"]
    dict set ondevice blocks $ondevice_blocks
    lappend schichten $erfassung
    
    # Bildverbesserungs-Schicht
    set verbesserung [dict create]
    dict set verbesserung name "Bildverbesserung"
    dict set verbesserung farbe "#6d5bd0"
    set verbesserung_blocks [list]
    lappend verbesserung_blocks [dict create id "clahe-4x" name "CLAHE 4x" untertitel "Kontrast"]
    lappend verbesserung_blocks [dict create id "unsharp-mask" name "Unsharp-Mask" untertitel "5 Stufen"]
    lappend verbesserung_blocks [dict create id "helligkeit-saettigung" name "Helligkeit/Saettigung" untertitel "Gegenlicht"]
    dict set verbesserung blocks $verbesserung_blocks
    lappend schichten $verbesserung
    
    # Cloud Vision-Schicht
    set cloud [dict create]
    dict set cloud name "Cloud Vision"
    dict set cloud farbe "#b45309"
    set cloud_blocks [list]
    lappend cloud_blocks [dict create id "gpt-4o" name "GPT-4o" untertitel "detail:high"]
    lappend cloud_blocks [dict create id "gemini-2-5" name "Gemini 2.5" untertitel "Bounding-Box"]
    lappend cloud_blocks [dict create id "claude" name "Claude" untertitel "Domaenenwissen"]
    lappend cloud_blocks [dict create id "worker-proxy" name "Worker-Proxy" untertitel "CORS fuer Claude"]
    dict set cloud blocks $cloud_blocks
    lappend schichten $cloud
    
    # Ausgabeschicht
    set ausgabe [dict create]
    dict set ausgabe name "Ausgabe"
    dict set ausgabe farbe "#0f766e"
    set ausgabe_blocks [list]
    lappend ausgabe_blocks [dict create id "pixel-inspektor" name "Pixel-Inspektor" untertitel "8x-Lupe"]
    lappend ausgabe_blocks [dict create id "overlay" name "Overlay" untertitel "Echtzeit"]
    lappend ausgabe_blocks [dict create id "pwa" name "PWA" untertitel "Service Worker"]
    dict set ausgabe blocks $ausgabe_blocks
    lappend schichten $ausgabe
    
    dict set spec schichten $schichten
    
    # Kanten definieren
    set kanten [list]
    lappend kanten [dict create von "getusermedia" nach "tensorflow-js" art "fluss"]
    lappend kanten [dict create von "4k-auswahl" nach "coco-ssd-v2" art "fluss"]
    lappend kanten [dict create von "geraete-dropdown" nach "webgl" art "fluss"]
    lappend kanten [dict create von "tensorflow-js" nach "clahe-4x" art "fluss"]
    lappend kanten [dict create von "coco-ssd-v2" nach "unsharp-mask" art "fluss"]
    lappend kanten [dict create von "webgl" nach "helligkeit-saettigung" art "fluss"]
    lappend kanten [dict create von "clahe-4x" nach "gpt-4o" art "fluss"]
    lappend kanten [dict create von "unsharp-mask" nach "gemini-2-5" art "fluss"]
    lappend kanten [dict create von "helligkeit-saettigung" nach "claude" art "fluss"]
    lappend kanten [dict create von "gpt-4o" nach "pixel-inspektor" art "fluss"]
    lappend kanten [dict create von "gemini-2-5" nach "overlay" art "fluss"]
    lappend kanten [dict create von "claude" nach "pwa" art "fluss"]
    lappend kanten [dict create von "worker-proxy" nach "pixel-inspektor" art "fluss"]
    dict set spec kanten $kanten
    
    # Kantenarten definieren
    set kantenarten [list]
    lappend kantenarten [dict create art "fluss" farbe "#0f766e" stil "voll" text "Fluss von unten nach oben"]
    dict set spec kantenarten $kantenarten
    
    return $spec
}

# Generiert das komplette HTML-Dokument für die 3D-Architekturvisualisierung
proc generate_html {output_file} {
    set spec_json [::json::write object {*}[get_spec_data]]
