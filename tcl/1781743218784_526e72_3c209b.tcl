...
proc main {} {
    ...
    set options {
        {OutputPath.arg "" "Output path for the HTML file"}
    }

    set usage "Usage: $::argv0 \[options\]\nOptions:"
    
    if {[catch {array set opts [cmdline::getoptions ::argv $options $usage]}]} {
        puts stderr "Error in command line arguments"
        exit 1
    }

    set output_path $opts(OutputPath)

    if {$output_path eq ""} {
        puts stderr "Missing required argument: OutputPath"
        exit 1
    }
