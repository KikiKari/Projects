...
function ConvertTo-JsonLite {
  ...
  if ($Data -is [Collections.IDictionary]) {
    $Pairs = foreach ($Key in ($Data.Keys | Sort-Object)) {
      '"{0}":{1}' -f $Key, (ConvertTo-JsonLite $Data[$Key])
    }
    return '{' + ($Pairs -join ',') + '}'
  }
  elseif ($Data -is [Array] -or $Data -is [Collections.IList]) {
    $Items = $Data | ForEach-Object { ConvertTo-JsonLite $_ }
    return '[' + ($Items -join ',') + ']'
  }
  elseif ($Data -eq $null) {
    return 'null'
  }
  elseif ($Data -is [Int] -or $Data -is [Double] -or $Data -is [Decimal]) {
    return "$Data"
  }
  else {
    $Escaped = "$Data".Replace('\', '\\').Replace('"', '\"').Replace("`n", '\n').Replace("`r", '\r').Replace("`t", '\t')
    return "`"$Escaped`""
  }
}
