...
<#
.SYNOPSIS
    Generates an HTML file named sidepanel.html with the specified structure and content.
.DESCRIPTION
    This PowerShell script creates a complete HTML document that represents the sidepanel.html file.
    It constructs the HTML using PowerShell's XML and string manipulation capabilities to ensure
    structural integrity and proper formatting.
.PARAMETER OutputPath
    The path where the generated HTML file will be saved. Defaults to "sidepanel.html" in the current directory.
.EXAMPLE
    .\Generate-SidePanel.ps1 -OutputPath "C:\temp\sidepanel.html"
#>

param(
    [string]$OutputPath = "sidepanel.html"
)
