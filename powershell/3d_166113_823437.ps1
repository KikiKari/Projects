#!/usr/bin/env pwsh
# 3d_166113.tcl — portiert nach powershell
# Quelle: tcl, Projects@abstractions:tcl/3d_166113.tcl
...
# Portierung von 3d.html nach PowerShell 7
# Erzeugt das HTML-Dokument dynamisch und schreibt es in eine Datei

function Write-3dHtml {
    param (
        [string]$Filename
    )
    
    $html = Generate-HtmlContent
...
<title>TikTok LIVE Companion Android — Interaktive Architektur</title>
<meta name="description" content="Die Schichtsicht der Android-App: WebView, Brücke, Compose, ShazamKit.">
...
<div class="wrap">
  ...
  <h1>TikTok LIVE Companion Android</h1>
  <p class="lede">Die Schichtsicht der Android-App: WebView, Brücke, Compose, ShazamKit.</p>
...
# Hauptprogramm
if ($args.Count -ne 1) {
    Write-Host "Verwendung: $($MyInvocation.MyCommand.Name) <ausgabedatei>"
    exit 1
}

$outputFile = $args[0]
Write-3dHtml -Filename $outputFile
