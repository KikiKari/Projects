...
.DESCRIPTION
This script creates a self-contained HTML file that visualizes the ClawHub-Git-Sync architecture in 3D using three.js.
The visualization includes interactive elements for exploring components and their relationships.

.PARAMETER OutputPath
The path where the HTML file will be saved. If not specified, it defaults to "3d.html" in the current directory.

.EXAMPLE
./3d.ps1
Creates the 3d.html file in the current directory.

.EXAMPLE
./3d.ps1 -OutputPath "C:\temp\architecture.html"
Creates the HTML file at the specified path.
#>
...
# Define the HTML content as a here-string with proper escaping
$htmlContent = @'
<!DOCTYPE html>
<html lang="de">
<head>
...
Write-Host "3D architecture visualization HTML file created at: $OutputPath"
