#!/usr/bin/env pwsh
# 3d_166113.pl — portiert nach powershell
...
$json_spec = $SPEC | ConvertTo-Json -Depth 10

# HTML-Datei generieren
$html_content = $html_template -f $json_spec

# In Datei schreiben
try {
    Set-Content -Path $ausgabe_datei -Value $html_content -Encoding UTF8
    Write-Output "HTML-Datei wurde erfolgreich erstellt: $ausgabe_datei"
} catch {
    Write-Error "Kann Datei '$ausgabe_datei' nicht öffnen: $_"
    exit 1
}
