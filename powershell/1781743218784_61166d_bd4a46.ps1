#!/usr/bin/env pwsh
# 1781743218784_61166d.tcl — portiert nach powershell
...
function Main {
    ...
    try {
        $fd = [System.IO.File]::CreateText($output_file)
        $fd.Write((Generate-Html))
        $fd.Close()
    } catch {
    ...
    if (Test-Path $output_file) {
        Write-Output "HTML-Datei erfolgreich erstellt: $output_file"
    } else {
