...
if len(sys.argv) != 2:
    print(f"Verwendung: {sys.argv[0]} <ausgabedatei>")
    sys.exit(1)
...
try:
    with open(ausgabe_datei, 'w', encoding='utf-8') as fh:
        fh.write(html_inhalt)
except IOError as e:
    print(f"Kann Datei '{ausgabe_datei}' nicht öffnen: {e}")
    sys.exit(1)

print(f"HTML-Datei wurde erfolgreich erstellt: {ausgabe_datei}")
