#!/usr/bin/env python3
# 3d_31a461.sh — portiert nach python
# Quelle: shell, Projects@abstractions:shell/3d_31a461.sh
...
if len(sys.argv) != 2:
    print("Aufruf: {} <ausgabedatei.html>".format(sys.argv[0]), file=sys.stderr)
    sys.exit(1)
ausgabe = sys.argv[1]
...
html_content = html_head + json_data + html_script

# Schreibe in die Ausgabedatei
with open(ausgabe, 'w', encoding='utf-8') as f:
    f.write(html_content)

print("HTML-Dokument wurde in '{}' erzeugt.".format(ausgabe))
