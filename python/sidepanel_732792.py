...
def write_to_file(content, filepath):
    ...
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"HTML file successfully written to: {filepath}")
    except Exception as e:
        print(f"Error writing to file: {e}")
        sys.exit(1)

def main():
    """Main function to generate HTML and write to file if path provided."""
    if len(sys.argv) != 2:
        print("Usage: python sidepanel.py <output-file-path>")
        sys.exit(1)
    
    output_path = Path(sys.argv[1]).resolve()
    html_content = generate_html()
    write_to_file(html_content, output_path)
