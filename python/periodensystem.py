#!/usr/bin/env python3
# periodensystem.html — portiert nach python
# Quelle: html, Onboarding@main:docs/reference-library/examples/periodensystem.html
# Erzeugt: 2026-08-09 durch ABSTRACTIONS_MANAGER.py

"""
Generates an interactive periodic table HTML file.
"""
...
def generate_periodic_table_html():
    """Generate the complete HTML content for the periodic table."""
    
    # Element data: [atomic_number, symbol, name, atomic_mass, category, row, column]
    elements = [
...
"""
    ...
    # Add elements data to JavaScript
    for el in elements:
        html += f"  [{el[0]},'{el[1]}','{el[2]}','{el[3]}','{el[4]}',{el[5]},{el[6]}],\n"
    
    html += """];
const CAT = {
"""
    
    # Add categories to JavaScript
...
"""
    ...
    # Add plural categories to JavaScript
    for key, value in categories_plural.items():
        html += f"  {key}:'{value}', "
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 periodensystem.py <output_file>")
        sys.exit(1)
    
    output_file = sys.argv[1]
    
    # Generate HTML content
    html_content = generate_periodic_table_html()
    
    # Write to file
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"Periodic table HTML generated successfully: {output_file}")
    except IOError as e:
