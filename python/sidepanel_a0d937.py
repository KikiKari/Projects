...
def main():
    """Main function to generate HTML and write it to a file."""
    if len(sys.argv) != 2:
        print("Verwendung: python sidepanel.py <ausgabedatei>", file=sys.stderr)
        sys.exit(1)

    output_file = sys.argv[1]
    html_content = generate_html()
    write_to_file(html_content, output_file)
    print(f"HTML erfolgreich in '{output_file}' geschrieben.")
