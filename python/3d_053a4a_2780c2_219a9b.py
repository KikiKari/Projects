#!/usr/bin/env python3
# 3d_053a4a_2780c2.pl — portiert nach python
...
def generate_html():
    ...
    html.append('<title>Python Hardener — Interaktive Architektur</title>')
    html.append('<meta name="description" content="Der Messplatz: zwei Läufe, dieselben Behauptungen, ein Ergebnis — drehen, zoomen, Knoten auswählen.">')
    html.append('<meta name="theme-color" content="#b45309">')
  ...
  *{box-sizing:border-box}
  ...
  a{color:var(--ac)}
    ...
    html.append('<p class="technik">three.js · r128</p>')
    html.append('<h1>Python Hardener</h1>')
    html.append('<p class="lede">Der Messplatz: zwei Läufe, dieselben Behauptungen, ein Ergebnis — drehen, zoomen, Knoten auswählen.</p>')
    
    html.append('<div class="raster">')
    html.append('<div class="buehne" id="buehne">')
    html.append('<div class="knoepfe">')
    html.append('<button id="btn-plus" title="Näher">+</button>')
    html.append('<button id="btn-minus" title="Weiter weg">−</button>')
    html.append('<button id="btn-reset">Zurücksetzen</button>')
    html.append('<button id="btn-iso" aria-pressed="true" title="Isometrisch oder perspektivisch">Iso</button>')
    html.append('</div>')
    html.append('</div>')
    
    html.append('<aside class="karte">')
    html.append('<h2>Ausgewählter Knoten</h2>')
    html.append('<h3 id="k-name">—</h3>')
    html.append('<p class="sub" id="k-sub">Knoten anklicken oder durchblättern</p>')
    html.append('<dl class="feld"><dt>Schicht</dt><dd id="k-schicht">—</dd></dl>')
    html.append('<dl class="feld"><dt>ID</dt><dd id="k-id">—</dd></dl>')
    html.append('<div class="blaettern">')
    html.append('<button id="btn-prev">←<br>Vorheriger</button>')
    html.append('<button id="btn-next">Nächster<br>→</button>')
    html.append('</div>')
    html.append('</aside>')
    html.append('</div>')
    
    html.append('<div class="legende" id="legende"></div>')
    html.append('<p class="fuss">Schematische Dokumentationsansicht — Blockgrößen messen weder Datenmenge noch Leistung. Keine Telemetrie, keine Fernabfragen: Die Seite lädt einmalig three.js vom CDN und rechnet danach ausschließlich lokal.</p>')
