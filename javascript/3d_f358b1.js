...
// Datenstruktur aus dem Original
const SPEC = {
  "schichten": [
    {
      "name": "Plattformen",
...
// HTML-Template mit eingebetteten Styles und Skripten
...
async function main() {
  ...
  if (args.length !== 1) {
    console.error('Verwendung: node 3d.js <ausgabedatei>');
    process.exit(1);
  }
  ...
  try {
    ...
    const readable = Readable.from([htmlContent]);
    
    await finished(readable.pipe(stream));
    console.log(`HTML-Datei erfolgreich erstellt: ${outputFile}`);
  } catch (error) {
    console.error('Fehler beim Erstellen der HTML-Datei:', error);
    process.exit(1);
  }
}
