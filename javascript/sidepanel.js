...
function generateCSS() {
  const styles = {
    ...
    '.count-button:hover, .count-button:focus-visible': {
    ...
    '.control-label output, .audio-meter-row strong': {
      'font-variant-numeric': 'tabular-nums'
    },
    'input[type="range"]': {
      'width': '100%',
    ...
    '.top-chatters-actions': {
      'display': 'flex',
      'justify-content': 'flex-end',
      'gap': '9px',
      'margin-top': '7px'
    },
    '.top-chatters-actions[hidden]': {
      'display': 'none'
    },
    '.top-chatter-link': {
      'padding': '0',
    ...
    '.song-result strong, .song-result span, .song-result a': {
      'display': 'block'
    },
    '.song-result a': {
      'margin-top': '4px',
    ...
    '.audience-list, .chat-history-list, .recommendation-list': {
      'display': 'grid',
      'gap': '7px',
      'margin-top': '9px'
    },
    ...
    '.auto-chat-refresh input[type="number"]': {
      'width': '48px',
      'margin-left': '3px',
      'padding': '3px 4px',
      'border': '1px solid var(--border)',
    ...
    '.quick-recover-setting input[type="number"]': {
      'width': '48px',
      'margin-left': '3px',
      'padding': '3px 4px',
      'border': '1px solid var(--border)',
    ...
    '.recommendation-controls': {
      'display': 'grid',
      'grid-template-columns': 'minmax(70px, .7fr) minmax(150px, 1.3fr)',
      'gap': '8px',
      'margin-top': '10px'
    },
    ...
    '.item-meta, .item-url, .caption-meta': {
      'color': 'var(--muted)',
      'font-size': '10px'
    },
    '.item-url': {
    ...
    '@media (min-width: 430px)': {
      ...
      '.speech-settings label:first-child': {
        'grid-column': '1 / -1'
      }
    },
  ...
  for (const [selector, rules] of Object.entries(styles)) {
    if (selector.startsWith('@media')) {
      ...
      for (const [subSelector, subRules] of Object.entries(rules)) {
        cssContent += `\n  ${subSelector} {`;
        for (const [property, value] of Object.entries(subRules)) {
          cssContent += `\n    ${property}: ${value};`;
        }
        cssContent += '\n  }';
      }
...
writeToFile(cssContent, outputFile);
