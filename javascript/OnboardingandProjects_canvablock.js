#!/usr/bin/env node
// OnboardingandProjects_canvablock.html — portiert nach javascript
...
// Helper function to create HTML elements as strings
function createElement(tag, attributes = {}, children = []) {
    ...
    const childrenContent = Array.isArray(children) ? children.join('') : children;
    return `<${tag}${attrString}>${childrenContent}</${tag}>`;
}

// Helper function to create script elements with content
function createScriptElement(content, attributes = {}) {
    return createElement('script', attributes, content);
}

// Helper function to create style elements with content
function createStyleElement(content) {
    return createElement('style', {}, content);
}

// Main function to generate the HTML document
function generateOnboardingAndProjectsDocument() {
    const htmlContent = `
<!DOCTYPE html>
<html>
<head>
<title>Onboarding and Projects</title>
<meta charset="utf-8">
