...
`;
  ...
  check_for_changes() {
    ...
    for (const doc of current_docs) {
      ...
      current_hashes[docPath] = doc.hash;
  ...
  cleanup_old_backups() {
    ...
    for (const db_name of ['docs.db', 'tree.db']) {
      const backups = fs.readdirSync(BACKUP_DIR)
        .filter(file => file.endsWith(`_${db_name}.bak`))
      ...
      for (const backup of backups) {
        try {
          ...
          const backupTime = new Date(`${dateStr}T${timeStr.replace(/-/g, ':')}`);
  ...
  run_cycle() {
    ...
    let last_backup = state.last_backup;
    
    let do_backup;
    if (last_backup) {
      const last_backup_time = new Date(last_backup);
      do_backup = (new Date() - last_backup_time) >= 60 * 60 * 1000; // 1 hour
    } else {
      do_backup = true;
    }
