...
function createHtmlDocument() {
  ...
  const head = {
    ...
    children: [
      ...
      { tag: 'meta', attributes: { name: 'theme-color', content: '#0d1b2a' } },
      { tag: 'meta', attributes: { 
        name: 'description', 
        content: 'Lokaler Regen-Check für die nächsten 30, 60 und 120 Minuten' 
      } },
      { tag: 'meta', attributes: { name: 'apple-mobile-web-app-capable', content: 'yes' } },
      { tag: 'meta', attributes: { 
        name: 'apple-mobile-web-app-status-bar-style', 
        content: 'black-translucent' 
      } },
      { tag: 'meta', attributes: { name: 'apple-mobile-web-app-title', content: 'Weather' } },
      { tag: 'link', attributes: { rel: 'manifest', href: './manifest.json' } },
      { tag: 'link', attributes: { rel: 'apple-touch-icon', href: './icon-512.png' } },
      { tag: 'title', content: 'Weather – Regen-Check' },
      { tag: 'script', attributes: { 
        type: 'module', 
        crossorigin: '', 
        src: './assets/index-DGdzG44S.js' 
      } },
      { tag: 'link', attributes: { 
        rel: 'stylesheet', 
        crossorigin: '', 
        href: './assets/index-erFv57XC.css' 
      } }
    ]
  };
  ...
  const body = {
    ...
    children: [
      ...
      {
        tag: 'script',
        attributes: { 'data-pplx-inline-edit': '' },
        content: `(function () {
...
function renderElement(element) {
  ...
  if (element.children) {
    ...
    return `<${element.tag}${attrs}>${childrenContent}</${element.tag}>`;
  }
...
function generateHtmlString(doc) {
  const htmlContent = renderElement(doc.html);
  return `${doc.docType}\n${htmlContent}\n`;
}
...
const htmlString = generateHtmlString(htmlDoc);
