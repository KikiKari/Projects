#!/usr/bin/env node
// 1781743218784_260531_8b95ef.tcl — portiert nach javascript
// Quelle: tcl, Projects@abstractions:tcl/1781743218784_260531_8b95ef.tcl
...
async function deriveKey(pw,salt){ const km=await crypto.subtle.importKey("raw",enc.encode(pw),"PBKDF2",false,["deriveKey"]); return crypto.subtle.deriveKey({name:"PBKDF2",salt,iterations:210000,hash:"SHA-256"},km,{name:"AES-GCM",length:256},false,["encrypt","decrypt"]); }
async function encryptObj(obj,pw){ const salt=crypto.getRandomValues(new Uint8Array(16)), iv=crypto.getRandomValues(new Uint8Array(12)); const key=await deriveKey(pw,salt); const ct=new Uint8Array(await crypto.subtle.encrypt({name:"AES-GCM",iv},key,enc.encode(JSON.stringify(obj,null,2))); const magic=enc.encode("SVPB1"); const out=new Uint8Array(5+16+12+ct.length); out.set(magic,0); out.set(salt,5); out.set(iv,21); out.set(ct,33); return u8b64(out); }
async function decryptB64(b64,pw){ const raw=b64u8(b64); if(dec.decode(raw.slice(0,5))!=="SVPB1") throw new Error("magic"); const key=await deriveKey(pw,raw.slice(5,21)); const pt=await crypto.subtle.decrypt({name:"AES-GCM",iv:raw.slice(21,33)},key,raw.slice(33)); return JSON.parse(dec.decode(pt)); }
function esc(s){return (s==null?"":String(s)).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); }

function renderEditor(){ document.getElementById("editor").classList.remove("hide"); document.getElementById("out").classList.remove("hide"); const P=VAULT.providers||{}; const root=document.getElementById("provs"); root.innerHTML=""; Object.keys(P).forEach(name=>{ const d=document.createElement("div"); d.className="prov"; let rows=""; Object.keys(P[name]).forEach(k=>{ rows+=\`<div class="kv"><span class="k">\${esc(k)}</span><input data-p="\${esc(name)}" data-k="\${esc(k)}" value="\${esc(P[name][k])}"><button class="btn sm" data-del="\${esc(name)}|\${esc(k)}">\${tr("del")}</button></div>\`; }); d.innerHTML=\`<h3>\${esc(name)} <button class="btn sm" data-delp="\${esc(name)}">\${tr("del")}</button></h3>\${rows}<div class="row" style="margin-top:6px"><input class="nf" data-np="\${esc(name)}" placeholder="\${tr("newField")}" style="max-width:180px"><input class="nv" data-np="\${esc(name)}" placeholder="\${tr("newValue")}" style="max-width:260px"><button class="btn sm" data-addf="\${esc(name)}">\${tr("addField")}</button></div>\`; root.appendChild(d); }); root.querySelectorAll("input[data-k]").forEach(i=>i.onchange=()=>{ VAULT.providers[i.dataset.p][i.dataset.k]=i.value; }); root.querySelectorAll("button[data-del]").forEach(b=>b.onclick=()=>{ const [p,k]=b.dataset.del.split("|"); delete VAULT.providers[p][k]; renderEditor(); }); root.querySelectorAll("button[data-delp]").forEach(b=>b.onclick=()=>{ delete VAULT.providers[b.dataset.delp]; renderEditor(); }); root.querySelectorAll("button[data-addf]").forEach(b=>b.onclick=()=>{ const p=b.dataset.addf; const nf=root.querySelector(\`.nf[data-np="\${CSS.escape(p)}"]\`).value.trim(); const nv=root.querySelector(\`.nv[data-np="\${CSS.escape(p)}"]\`).value; if(nf){ VAULT.providers[p][nf]=nv; renderEditor(); } }); }
...
openBtn.onclick=async()=>{ const m=document.getElementById("openMsg"); m.className="msg"; m.textContent=""; if(!pass.value){ m.className="msg err"; m.textContent=tr("needPass"); return; } if(!blob.value.trim()){ m.className="msg err"; m.textContent=tr("noInput"); return; } try{ VAULT=await decryptB64(blob.value,pass.value); if(!VAULT.providers)VAULT.providers={}; renderEditor(); m.className="msg ok"; m.textContent=tr("opened"); } catch(err){ m.className="msg err"; m.textContent=tr("bad"); } };
newBtn.onclick=()=>{ const m=document.getElementById("openMsg"); if(!pass.value){ m.className="msg err"; m.textContent=tr("needPass"); return; } VAULT={meta:{created:new Date().toISOString().slice(0,10),format:"SVPB1"},providers:{}}; renderEditor(); m.className="msg ok"; m.textContent=tr("created"); };
...
encBtn.onclick=async()=>{ const m=document.getElementById("saveMsg"); m.className="msg"; if(!VAULT){ m.className="msg err"; m.textContent=tr("needOpen"); return; } if(!pass.value){ m.className="msg err"; m.textContent=tr("needPass"); return; } result.value=await encryptObj(VAULT,pass.value); m.className="msg ok"; m.textContent=tr("encrypted"); };
