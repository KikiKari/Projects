...
function generateCSS() {
  ...
  css += ':root {\n';
  for (const [prop, value] of Object.entries(styles[':root'])) {
    css += `  ${prop}: ${value};\n`;
  }
  css += '}\n\n';

  // Generate regular styles
  ...
  for (const mq of mediaQueries) {
    css += `\n@media ${mq.condition} {`;
    for (const [selector, rules] of Object.entries(mq.rules)) {
      css += ` ${selector} {`;
      for (const [prop, value] of Object.entries(rules)) {
        css += ` ${prop}: ${value};`;
      }
...
function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: node generate-css.js <output-file>');
    process.exit(1);
  }
