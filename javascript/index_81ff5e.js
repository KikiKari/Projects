  ...
  const doc = {
    ...
    html: {
      ...
      head: {
        ...
        title: 'Vault Cache',
        style: `:root{ color-scheme:light; --ink:#1b1c1f; --muted:#6c6e75; --faint:#9a9ca3; --card:#fff; --line:#e9eaee; --accent:#5b5bd6; --accent2:#7c5cff; --ok:#22a06b; --err:#e0533d; --radius:16px; --shadow:0 1px 2px rgba(20,20,40,.04),0 6px 20px rgba(20,20,40,.06);}
...
a{color:var(--accent);}`
          ...
          children: [
            {
              div: {
                ...
                children: [
                  { div: { className: 'mark' } },
                  { h1: { id: 'title', text: 'Vault Cache' } }
                ]
              }
            },
            { div: { className: 'sub', id: 'sub', text: 'Verschlüsselte Secret-Vault (AES-256-GCM, PBKDF2) — alles im Browser, kein Server.' } },
            {
              div: {
                className: 'card',
                children: [
                  { h2: { id: 'h-open', text: 'Öffnen oder neu' } },
                  { label: { className: 'lab', id: 'l-pass', text: 'Passphrase' } },
                  { input: { id: 'pass', type: 'password', placeholder: 'Passphrase…' } },
                  { label: { className: 'lab', id: 'l-file', text: 'Vault laden (Datei oder Base64 einfügen)' } },
                  { input: { id: 'file', type: 'file', accept: '.svpb,.txt,.vault,.b64' } },
                  { textarea: { id: 'blob', placeholder: '…oder Base64 hier einfügen' } },
                  {
                    div: {
                      className: 'row',
                      style: 'margin-top:10px',
                      children: [
                        { button: { className: 'btn primary', id: 'openBtn', text: 'Öffnen / Entschlüsseln' } },
                        { button: { className: 'btn', id: 'newBtn', text: 'Neuer leerer Vault' } },
                        { span: { className: 'msg', id: 'openMsg' } }
                      ]
                    }
                  }
                ]
                  ...
                  { div: { id: 'provs' } },
                  {
                    div: {
                      ...
                      children: [
                        ...
                        { button: { className: 'btn sm', id: 'addProvBtn', text: '+ Anbieter' } }
                      ]
                    }
                  }
                ]
            ...
            },
            {
              div: {
                ...
                id: 'out',
                children: [
                  { h2: { id: 'h-save', text: 'Speichern / Export' } },
                  {
                    div: {
                      className: 'row',
                      children: [
                        { button: { className: 'btn primary', id: 'encBtn', text: 'Verschlüsseln' } },
                        { button: { className: 'btn', id: 'dlBtn', text: 'Als .svpb herunterladen' } },
                        { button: { className: 'btn', id: 'expBtn', text: 'Klartext-JSON exportieren' } },
                        { span: { className: 'msg', id: 'saveMsg' } }
                      ]
                    }
                  },
                  { label: { className: 'lab', id: 'l-result', text: 'Ergebnis (zum Kopieren/Speichern)' } },
                  { textarea: { id: 'result', readonly: true } }
                ]
              }
            },
  ...
  // Add script tag with JSON metadata
  ...
  // Add main script
  doc.html.body.script = `const L = ((navigator.language||"en").toLowerCase().startsWith("de"))?"de":"en";
const T = {
...
function renderElement(tag, attributes = {}, content = '') {
...
function buildHTML(doc) {
  ...
  // Add script tag with JSON metadata
  html += `  <script type="${doc.html.head.script.type}" id="${doc.html.head.script.id}">\n`;
  html += doc.html.head.script.text;
  html += '\n  </script>\n';
  ...
  // Build body
  html += '<body>\n';
  html += buildElement(doc.html.body.div);
  html += '\n';
  html += '  <script>\n';
  html += doc.html.body.script;
  html += '\n  </script>\n';
  html += '</body>\n';
  html += '</html>';
...
function buildElement(element) {
  ...
  if (element.children) {
    ...
    const attrs = element[tag];
    
    if (tag === 'children') {
      return attrs.map(child => buildElement(child)).join('\n');
    }
    ...
    if (attrs.text && !attrs.children) {
      html += `>${attrs.text}</${tag}>`;
    } else if (attrs.children) {
      html += '>\n';
      html += attrs.children.map(child => '    ' + buildElement(child)).join('\n');
      html += `\n  </${tag}>`;
    } else {
      html += `></${tag}>`;
    }
