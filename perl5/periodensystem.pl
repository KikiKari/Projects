...
my $output_file = shift @ARGV or die "Bitte geben Sie den Namen der Ausgabedatei an.\n";

# Öffne die Ausgabedatei zum Schreiben
open my $fh, '>', $output_file or die "Konnte '$output_file' nicht öffnen: $!\n";

# Schreibe den HTML-Header
print $fh <<'EOF';
<!DOCTYPE html>
<html lang="de">
<head>
...
# Definiere die Kategorien im Plural
my %categories_plural = (
  am => 'Alkalimetalle',
  em => 'Erdalkalimetalle',
  tm => 'Übergangsmetalle',
  ptm => 'Metalle (Hauptgr.)',
...
# Schreibe die JavaScript-Funktionen
print $fh <<'EOF';
...
# Erstelle das JavaScript-Array für die Elemente
print $fh "const E = [\n";
for my $element (@elements) {
  my ($z, $sym, $name, $mass, $cat, $row, $col) = @$element;
  print $fh "  [$z,'$sym','$name','$mass','$cat',$row,$col],\n";
}
print $fh "];\n";

# Erstelle das JavaScript-Objekt für die Kategorien
print $fh "const CAT = {\n";
for my $key (sort keys %categories) {
  print $fh "  $key:'$categories{$key}',\n";
}
print $fh "};\n";

# Erstelle das JavaScript-Objekt für die Kategorien im Plural
print $fh "const CATPL = {\n";
for my $key (sort keys %categories_plural) {
  print $fh "  $key:'$categories_plural{$key}',\n";
}
print $fh "};\n";

# Schreibe den Rest des JavaScript-Codes
print $fh <<'EOF';
