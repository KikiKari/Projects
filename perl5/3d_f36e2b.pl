...
my $output_path;
GetOptions('output-path=s' => \$output_path) or die "Ungültige Optionen\n";

if (!$output_path) {
    die "Fehlender Parameter: --output-path\n";
}
...
open my $fh, '>:encoding(UTF-8)', $output_path or die "Konnte '$output_path' nicht öffnen: $!";
print $fh $html_content;
close $fh;

print "HTML-Datei erfolgreich generiert unter: $output_path\n";
