#!/usr/bin/env perl
# 3d_053a4a_2780c2.ps1 — portiert nach perl5
# Quelle: powershell, Projects@abstractions:powershell/3d_053a4a_2780c2.ps1
...
sub generate_html {
    ...
    push @html, '<html lang="de">';
    
    # Head section
    push @html, '<head>';
    push @html, '<meta charset="utf-8">';
    push @html, '<meta name="viewport" content="width=device-width, initial-scale=1">';
    push @html, '<title>Python Hardener — Interaktive Architektur</title>';
    push @html, '<meta name="description" content="Der Messplatz: zwei Läufe, dieselben Behauptungen, ein Ergebnis — drehen, zoomen, Knoten auswählen.">';
    push @html, '<meta name="theme-color" content="#b45309">';
...
CSS_END
    ...
    push @html, '<p class="technik">three.js · r128</p>';
    push @html, '<h1>Python Hardener</h1>';
    push @html, '<p class="lede">Der Messplatz: zwei Läufe, dieselben Behauptungen, ein Ergebnis — drehen, zoomen, Knoten auswählen.</p>';
    
    push @html, '<div class="raster">';
    push @html, '<div class="buehne" id="buehne">';
    push @html, '<div class="knoepfe">';
    push @html, '<button id="btn-plus" title="Näher">+</button>';
    push @html, '<button id="btn-minus" title="Weiter weg">−</button>';
    push @html, '<button id="btn-reset">Zurücksetzen</button>';
    push @html, '<button id="btn-iso" aria-pressed="true" title="Isometrisch oder perspektivisch">Iso</button>';
    push @html, '</div>';
    push @html, '</div>';
    
    push @html, '<aside class="karte">';
    push @html, '<h2>Ausgewählter Knoten</h2>';
    push @html, '<h3 id="k-name">—</h3>';
    push @html, '<p class="sub" id="k-sub">Knoten anklicken oder durchblättern</p>';
    push @html, '<dl class="feld"><dt>Schicht</dt><dd id="k-schicht">—</dd></dl>';
    push @html, '<dl class="feld"><dt>ID</dt><dd id="k-id">—</dd></dl>';
    push @html, '<div class="blaettern">';
    push @html, '<button id="btn-prev">←<br>Vorheriger</button>';
    push @html, '<button id="btn-next">Nächster<br>→</button>';
    push @html, '</div>';
    push @html, '</aside>';
    push @html, '</div>';
    
    push @html, '<div class="legende" id="legende"></div>';
    push @html, '<p class="fuss">Schematische Dokumentationsansicht — Blockgrößen messen weder Datenmenge noch Leistung. Keine Telemetrie, keine Fernabfragen: Die Seite lädt einmalig three.js vom CDN und rechnet danach ausschließlich lokal.</p>';
    
    push @html, '</div>';
    
    # External script
    push @html, '<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>';
