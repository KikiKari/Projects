#!/usr/bin/env perl
# 3d_111adf.tcl — portiert nach perl5
# Quelle: tcl, Projects@abstractions:tcl/3d_111adf.tcl
...
if ($@) {
    print STDERR "Error generating HTML: $@\n";
    exit 1;
}

open(my $fh, '>', $output_file) or do {
    print STDERR "Error opening file '$output_file': $!\n";
    exit 1;
};

print $fh $html_content or do {
    print STDERR "Error writing to file '$output_file': $!\n";
    close $fh;
    exit 1;
};

close $fh or do {
    print STDERR "Error closing file '$output_file': $!\n";
    exit 1;
};
