...
if ANDROID.exists():
    ...
    android_main = (ANDROID / "app" / "src" / "main" / "java" / "app" / "tiktoklivecompanion" / "MainActivity.kt").read_text(encoding="utf-8")
    require('minSdk = 21' in gradle and 'versionName = "0.7.1"' in gradle, "Android version contract")
    ...
    require('org.videolan.android:libvlc-all:3.7.5' in gradle, "pinned stable Android LibVLC")
    require(android_main.index('Text("VLC Ersatz")') < android_main.index('Text("VLC Player")'), "Android VLC button order")
...
if IOS.exists():
    ...
    ios_content = (IOS / "TikTokLiveCompanion" / "ContentView.swift").read_text(encoding="utf-8")
    ...
    require("MARKETING_VERSION = 0.7.1" in pbx and "IPHONEOS_DEPLOYMENT_TARGET = 15.0" in pbx, "iOS version contract")
    ...
    require("VlcVideoSurface.swift in Sources" in pbx, "iOS VLC source membership")
    require("pod 'MobileVLCKit', '3.7.3'" in podfile, "pinned stable iOS MobileVLCKit")
    require(ios_content.index('Button("VLC Ersatz"') < ios_content.index('Button("VLC Player"'), "iOS VLC button order")
    ...
    require(info["CFBundleShortVersionString"] == "0.7.1", "iOS plist version")
