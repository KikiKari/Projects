...
assert.ok(contentSource.includes("const QUICK_RECOVER_RELOAD_COOLDOWN_MS = 400;"));
...
assert.ok(panelHtml.indexOf('id="game-mode"') < panelHtml.indexOf('id="auto-chat-refresh"'));
...
assert.ok(setupSource.includes("Den individuellen Pairing-Code oben markieren und mit STRG+C kopieren."));
assert.ok(setupSource.includes("Danach im Sidepanel in das Feld Pairing-Code klicken und mit STRG+V einfuegen."));
...
assert.ok(sidepanelSource.includes("setIntegrationFieldsHidden(Boolean(health.sherpaConfigured))"));
assert.ok(panelCss.includes(".settings-grid label[hidden] { display: none; }"));
...
assert.ok(sidepanelSource.includes('send("TLC_START_LOCAL_SERVICE", { nonce })'));
...
assert.ok(backgroundSource.includes("Date.now() - lastAt < 400"));
