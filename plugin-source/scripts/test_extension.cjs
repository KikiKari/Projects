...
assert.ok(sidepanelSource.includes('send("TLC_INSTALL_LOCAL_SERVICE")'));
assert.ok(backgroundSource.includes("tiktok-live-companion://install?nonce="));
assert.ok(backgroundSource.includes("extensionId=${encodeURIComponent(chrome.runtime.id)}"));
...
assert.ok(setupSource.includes('$runningService.bootstrapPairing'));
...
assert.ok(backgroundSource.includes("tiktok-live-companion://start?nonce="));
...
assert.ok(backgroundSource.includes("chrome.tabs.remove(serviceTab.id)"));
