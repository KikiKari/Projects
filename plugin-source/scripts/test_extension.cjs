...
assert.strictEqual(manifest.content_scripts[0].js[0], "vendor-mpegts.js");
const mpegtsVendorPath = path.join(extension, "vendor-mpegts.js");
const mpegtsLicensePath = path.join(extension, "vendor-mpegts.LICENSE.txt");
const mpegtsNoticePath = path.join(extension, "vendor-mpegts.NOTICE.md");
assert.ok(fs.existsSync(mpegtsVendorPath));
assert.ok(fs.existsSync(mpegtsLicensePath));
assert.ok(fs.existsSync(mpegtsNoticePath));
assert.strictEqual(
  crypto.createHash("sha256").update(fs.readFileSync(mpegtsVendorPath)).digest("hex").toUpperCase(),
  "0786F9AF6780822FF29240259A73B07ED7BC479BC44966E49418DD38213B8064"
);
...
const repairSource = fs.readFileSync(path.join(root, "companion-service", "Sprachdienst-reparieren.cmd"), "utf8");
const repairPowerShellSource = fs.readFileSync(path.join(root, "companion-service", "repair-service.ps1"), "utf8");
assert.ok(/const MAX_CHAT = 500;/.test(backgroundSource));
...
assert.ok(backgroundSource.includes("universalCaptionApiKeyConfigured"));
...
assert.ok(backgroundSource.includes('case "TLC_SET_DEBUG"'));
...
assert.ok(backgroundSource.includes("return { armed: Boolean(enabled), waitingForTikTok: Boolean(enabled), reloading: false }"));
...
assert.ok(contentSource.includes("const POPUP_GUARD_GRACE_MS = 40;"));
assert.ok(contentSource.includes("const QUICK_RECOVER_INTERVAL_MS = 40;"));
assert.ok(contentSource.includes("const QUICK_RECOVER_CONFIRM_MS = 3000;"));
...
assert.ok(backgroundSource.includes("quickRecoverInFlight.has(tabId)"));
...
assert.ok(contentSource.includes("attemptedMediaFallbackUrls"));
assert.ok(contentSource.includes("globalThis.mpegts.createPlayer"));
assert.ok(contentSource.includes("enableWorker: false"));
assert.ok(contentSource.includes('setTimeout(() => done(false, "timeout"), 15000)'));
assert.ok(contentSource.includes("Erneuter Klick prüft die nächste ungetestete Quelle."));
assert.ok(contentSource.includes("playerBonus"));
assert.ok(contentSource.includes("videoArea * 4.5"));
...
assert.ok(sidepanelSource.includes('type: "TLC_SET_TAB_SPEECH"'));
assert.ok(backgroundSource.includes('case "TLC_SET_TAB_SPEECH"'));
assert.ok(backgroundSource.includes("ensureOffscreenDocument"));
...
assert.ok(contentSource.includes("function sponsoredContentBadgePresent"));
assert.ok(contentSource.includes("sponsoredContentSeenHandle"));
assert.ok(contentSource.includes("Werbeinhalt"));
assert.ok(contentSource.includes("sponsoredContentLabel"));
assert.ok(contentSource.includes("function paidPartnershipBadgePresent"));
assert.ok(contentSource.includes("paidPartnershipSeenHandle"));
assert.ok(contentSource.includes("Bezahlte Partnerschaft"));
assert.ok(contentSource.includes("paidPartnershipLabel"));
...
assert.ok(panelHtml.includes('min="1" max="59"'));
const panelCss = fs.readFileSync(path.join(extension, "sidepanel.css"), "utf8");
const offscreenSource = fs.readFileSync(path.join(extension, "offscreen.js"), "utf8");
assert.ok(manifest.permissions.includes("offscreen"));
assert.ok(offscreenSource.includes("TLC_OFFSCREEN_SPEAK"));
assert.ok(offscreenSource.includes("TLC_OFFSCREEN_CANCEL"));
assert.ok(panelHtml.indexOf('id="page-info-heading"') > panelHtml.indexOf('id="top-chatters-heading"'));
assert.ok(panelHtml.indexOf('id="page-info-heading"') < panelHtml.indexOf('id="hook-heading"'));
assert.ok(panelHtml.indexOf('id="caption-heading"') > panelHtml.indexOf('id="hook-heading"'));
assert.ok(panelHtml.indexOf('id="song-heading"') > panelHtml.indexOf('id="player-heading"'));
...
assert.ok(panelHtml.includes('id="top-chatters-more"'));
assert.ok(panelHtml.includes('id="top-chatters-reset"'));
...
assert.ok(panelHtml.indexOf('id="hook-heading"') < panelHtml.indexOf('id="recommendations-section"'));
assert.ok(panelHtml.indexOf('id="recommendations-section"') < panelHtml.indexOf('id="caption-heading"'));
...
assert.ok(panelHtml.includes('id="chat-history-modal"'));
assert.ok(panelHtml.includes('id="close-chat-history"'));
assert.ok(panelHtml.includes('id="auto-chat-refresh"'));
assert.ok(panelHtml.includes('id="auto-chat-refresh-minutes"'));
assert.ok(panelHtml.includes("Chatnamen sprechen"));
assert.ok(panelHtml.indexOf('id="speak-names"') < panelHtml.indexOf('id="shorten-names"'));
assert.ok(panelHtml.indexOf('id="shorten-names"') < panelHtml.indexOf('id="game-mode"'));
assert.ok(panelHtml.indexOf('id="auto-chat-refresh"') < panelHtml.indexOf('id="speech-settings-modal"'));
assert.ok(panelHtml.indexOf('id="speech-settings-modal"') < panelHtml.indexOf('id="game-mode"'));
assert.ok(panelHtml.includes('id="speech-language"'));
assert.ok(panelHtml.includes('id="speech-voice"'));
assert.ok(panelHtml.indexOf('id="speech-language"') > panelHtml.indexOf('id="speech-settings-modal"'));
assert.ok(panelHtml.indexOf('id="speech-voice"') > panelHtml.indexOf('id="speech-settings-modal"'));
assert.ok(panelHtml.includes('id="audd-token"'));
assert.ok(panelHtml.includes("AudD API-Token"));
assert.ok(panelHtml.includes('href="https://audd.io/"'));
assert.ok(panelHtml.includes('target="_blank"'));
assert.ok(panelHtml.includes('rel="noopener noreferrer"'));
assert.ok(sidepanelSource.includes("AudD API-Token Trail Plan (Songerkennung)"));
assert.ok(sidepanelSource.includes("AudD API-Token (Songerkennung)"));
assert.ok(sidepanelSource.includes('link.href = "https://audd.io/"'));
assert.ok(sidepanelSource.includes('link.target = "_blank"'));
assert.ok(sidepanelSource.includes('elements["service-action"].disabled = true'));
assert.ok(setupSource.includes("Der Pairing-Code wird automatisch an die anfragende Erweiterung uebergeben."));
assert.ok(setupSource.includes("Nur falls die Kopplung nicht bestaetigt wird"));
assert.ok(setupSource.includes("Remove-Item -LiteralPath $generatedScriptPath -Force"));
assert.ok(repairSource.includes('-File "%~dp0repair-service.ps1"'));
assert.ok(repairSource.includes("Sprachdienst starten"));
assert.ok(repairPowerShellSource.includes("service.json"));
assert.ok(repairPowerShellSource.includes("Read-Host \"Erweiterungs-ID\""));
assert.ok(repairPowerShellSource.includes('Join-Path $serviceDir "setup.ps1"'));
assert.ok(setupSource.includes("Vorhandener Sprachdienst wurde fuer die saubere Neueinrichtung beendet."));
assert.ok(!setupSource.includes("-and -not $runningService.bootstrapPairing"));
...
assert.ok(panelHtml.includes("Installation abschließen!"));
assert.ok(!sidepanelSource.includes("navigator.clipboard.writeText(setupCommand)"));
assert.ok(sidepanelSource.includes('send("TLC_INSTALL_LOCAL_SERVICE", { nonce })'));
assert.ok(sidepanelSource.includes("attempt < 180"));
assert.ok(sidepanelSource.includes("Sprachdienst-reparieren.cmd"));
assert.ok(sidepanelSource.includes('openServiceProtocol("install", nonce)'));
assert.ok(!sidepanelSource.includes('openServiceProtocol("start", nonce)'));
assert.ok(sidepanelSource.includes('openServiceProtocol("install", nonce)'));
assert.ok(sidepanelSource.includes("active: true"));
assert.ok(sidepanelSource.includes("CMD richtet den vorhandenen oder neuen Sprachdienst ein"));
assert.ok(sidepanelSource.includes('send("TLC_POLL_LOCAL_SERVICE_INSTALL")'));
assert.ok(backgroundSource.includes('case "TLC_POLL_LOCAL_SERVICE_INSTALL"'));
assert.ok(backgroundSource.includes("expectedProtocolHandler"));
assert.ok(backgroundSource.includes("installationPending"));
assert.ok(backgroundSource.includes("pairingConfigured"));
assert.ok(backgroundSource.includes("auddTokenConfigured"));
assert.ok(backgroundSource.includes("localService,"));
assert.ok(backgroundSource.includes("SERVICE_INSTALL_KEY"));
assert.ok(backgroundSource.includes("knownPairingCode"));
assert.ok(backgroundSource.includes("Authorization: `Bearer ${knownPairingCode}`"));
assert.ok(sidepanelSource.includes("tiktok-live-companion://${mode}/${path}"));
assert.ok(sidepanelSource.includes("${encodeURIComponent(nonce)}/${encodeURIComponent(chrome.runtime.id)}"));
assert.ok(setupSource.includes('[string]$BootstrapNonce = ""'));
assert.ok(setupSource.includes('Set-Location -LiteralPath "$serviceDir"'));
assert.ok(setupSource.includes("Get-Command pwsh.exe"));
assert.ok(setupSource.includes('protocol-handler.cmd'));
assert.ok(setupSource.includes('powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$setupScriptPath"'));
assert.ok(!setupSource.includes('call "$npmPath" run setup'));
assert.ok(setupSource.includes("Pairing-Code wird automatisch"));
assert.ok(!setupSource.includes('$serviceRoot ='));
assert.ok(!setupSource.includes('call "$npmPath" run setup --'));
...
assert.ok(panelHtml.includes('>JSON-L-Export</button>'));
assert.ok(panelHtml.includes('>RAW-JSON-Export</button>'));
...
assert.ok(sidepanelSource.includes('["Kyrillisch"'));
assert.ok(sidepanelSource.includes('["Asiatisch"'));
assert.ok(sidepanelSource.includes('["Abjad"'));
assert.ok(sidepanelSource.includes('["Indisch"'));
assert.ok(sidepanelSource.indexOf('"sherpa-de-eva-k"') < sidepanelSource.indexOf('"sherpa-ar-kareem"'));
assert.ok(sidepanelSource.includes("function renderChatHistory"));
assert.ok(sidepanelSource.includes("slice(-500).reverse()"));
assert.ok(sidepanelSource.includes("function scheduleAutoChatRefresh"));
assert.ok(sidepanelSource.includes('clearChatDisplay().catch'));
assert.ok(backgroundSource.includes("const MAX_CHAT = 500"));
assert.ok(sidepanelSource.includes("/v1/config/audd-token"));
assert.ok(sidepanelSource.includes("auddApiToken = response.settings?.auddApiToken"));
assert.ok(sidepanelSource.includes("TLC_SET_SPEECH_PREFERENCE\", { auddApiToken: token }"));
assert.ok(sidepanelSource.indexOf('/v1/config/audd-token') < sidepanelSource.indexOf('TLC_SET_SPEECH_PREFERENCE\", { auddApiToken: token }'));
assert.ok(sidepanelSource.includes("Pairing-Code wurde nicht gespeichert"));
assert.ok(!sidepanelSource.includes("setIntegrationFieldsHidden"));
assert.ok(panelHtml.includes('id="open-speech-settings"'));
assert.ok(panelHtml.includes('id="speech-settings-modal"'));
assert.ok(panelHtml.includes('id="close-speech-settings"'));
assert.ok(panelHtml.includes('id="universal-caption-api-key"'));
assert.ok(panelHtml.indexOf('id="audd-token"') > panelHtml.indexOf('id="speech-settings-modal"'));
assert.ok(panelHtml.indexOf('id="pairing-code"') > panelHtml.indexOf('id="speech-settings-modal"'));
assert.ok(sidepanelSource.includes('elements["speech-settings-modal"].hidden = false'));
assert.ok(backgroundSource.includes("universalCaptionApiKey"));
assert.ok(panelHtml.includes('id="export-caption-raw"'));
assert.ok(sidepanelSource.includes('send("TLC_GET_CAPTION_RAW_EXPORT")'));
assert.ok(backgroundSource.includes('case "TLC_GET_CAPTION_RAW_EXPORT"'));
assert.ok(backgroundSource.includes('schema: "tiktok-live-companion-caption-raw-v1"'));
for (const field of ["text", "data", "sources", "languages", "messages", "dataStreams", "playerTexts", "captionProtocol"]) {
  assert.ok(backgroundSource.includes(`${field}:`), `RAW export field missing: ${field}`);
}
assert.ok(!sidepanelSource.includes('elements["audd-token"].value = "";'));
...
assert.ok(sidepanelSource.includes("liveProLabel"));
assert.ok(sidepanelSource.includes("livePageUrl"));
assert.ok(sidepanelSource.includes("sponsoredContentLabel"));
assert.ok(sidepanelSource.includes("cleanSpeechPayload"));
assert.ok(sidepanelSource.includes("function hasGermanSpecialChars"));
assert.ok(sidepanelSource.includes('return "de-DE"'));
...
assert.ok(backgroundSource.includes("auddApiToken"));
assert.ok(sidepanelSource.includes('send("TLC_INSTALL_LOCAL_SERVICE", { nonce })'));
assert.ok(sidepanelSource.includes('send("TLC_POLL_LOCAL_SERVICE_INSTALL")'));
assert.ok(sidepanelSource.includes('elements["pairing-code"].value = pairingCode'));
assert.ok(backgroundSource.includes("/v1/pair?nonce="));
assert.ok(!backgroundSource.includes("npm run setup -- -ExtensionId ${chrome.runtime.id}"));
assert.ok(backgroundSource.includes('sender.url !== chrome.runtime.getURL("offscreen.html")'));
...
assert.ok(backgroundSource.includes("function resetPageIdentityIfChanged"));
assert.ok(backgroundSource.includes("function profileMatchesHandle"));
assert.ok(backgroundSource.includes("const targetHandle = normalizeHandle(match[1])"));
assert.ok(backgroundSource.includes("resetPageIdentityIfChanged(state, nextHandle)"));
...
assert.ok(contentSource.includes('setTimeout(() => done(false, "timeout"), 15000)'));
assert.ok(contentSource.includes("attemptedMediaFallbackUrls"));
assert.ok(contentSource.includes("activeMediaFallbackItem"));
assert.ok(contentSource.includes("activeMediaFallbackPlayer"));
assert.ok(contentSource.includes("globalThis.mpegts.createPlayer"));
assert.ok(contentSource.includes('type: "flv", isLive: true'));
assert.ok(contentSource.includes("destroyMediaFallbackPlayer"));
assert.ok(contentSource.includes("Alle erkannten Video-Links wurden bereits getestet."));
assert.ok(contentSource.includes("Die letzte funktionierende Quelle wurde wiederhergestellt."));
assert.ok(contentSource.includes("Erneuter Klick prüft die nächste ungetestete Quelle"));
