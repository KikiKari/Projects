#!/bin/bash
# 3d.py — portiert nach shell
# Quelle: python, Projects@abstractions:python/3d.py
...
# Funktion zur Erstellung der Spezifikation
get_spec() {
    cat <<'EOF'
{
  "schichten": [
...
# Hauptausführung
spec=$(get_spec)
# Escape single quotes for safe insertion into JavaScript
spec_escaped=$(printf '%s' "$spec" | sed "s/'/\\'/g")
