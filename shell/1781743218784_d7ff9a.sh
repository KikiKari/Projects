...
function renderEditor(){
  ...
  Object.keys(P).forEach(name=>{
    ...
    d.innerHTML=`<h3>${esc(name)} <button class="btn sm" data-delp="${esc(name)}">${tr("del")}</button></h3>${rows}
      <div class="row" style="margin-top:6px"><input data-newk="${esc(name)}" placeholder="${tr("newField")}" style="max-width:140px"><input data-newv="${esc(name)}" placeholder="${tr("newValue")}"><button class="btn sm" data-addk="${esc(name)}">${tr("addField")}</button></div>`;
    root.appendChild(d);
  });
  document.querySelectorAll("input[data-p]").forEach(el=>el.addEventListener("input",e=>{
    const p=el.dataset.p, k=el.dataset.k;
    VAULT.providers[p][k]=el.value;
  }));
  document.querySelectorAll("button[data-del]").forEach(el=>el.addEventListener("click",e=>{
    const [p,k]=el.dataset.del.split("|");
    delete VAULT.providers[p][k];
    renderEditor();
  }));
  document.querySelectorAll("button[data-delp]").forEach(el=>el.addEventListener("click",e=>{
    delete VAULT.providers[el.dataset.delp];
    renderEditor();
  }));
  document.querySelectorAll("button[data-addk]").forEach(el=>el.addEventListener("click",e=>{
    const p=el.dataset.addk;
    const k=document.querySelector(`input[data-newk="${esc(p)}"]`).value;
    const v=document.querySelector(`input[data-newv="${esc(p)}"]`).value;
    if(k && v){
...
document.getElementById("newBtn").addEventListener("click",e=>{
  ...
  renderEditor();
  openMsg.textContent=tr("created");
...
document.getElementById("openBtn").addEventListener("click",async e=>{
  ...
  if(file) b64=await file.text();
  else b64=document.getElementById("blob").value.trim();
  if(!b64){ openMsg.textContent=tr("noInput"); openMsg.className="msg err"; return; }
  try{
    VAULT=await decryptB64(b64,pw);
    renderEditor();
    openMsg.textContent=tr("opened");
    openMsg.className="msg ok";
  }catch(ex){
    openMsg.textContent=tr("bad");
    openMsg.className="msg err";
  }
});

document.getElementById("encBtn").addEventListener("click",async e=>{
  ...
  try{
    document.getElementById("result").value=await encryptObj(VAULT,pw);
    saveMsg.textContent=tr("encrypted");
...
document.getElementById("dlBtn").addEventListener("click",e=>{
  ...
  a.href="data:application/octet-stream;base64,"+document.getElementById("result").value;
...
document.getElementById("expBtn").addEventListener("click",e=>{
  if(!VAULT){ saveMsg.textContent=tr("needOpen"); saveMsg.className="msg err"; return; }
  document.getElementById("result").value=JSON.stringify(VAULT,null,2);
});

document.getElementById("addProvBtn").addEventListener("click",e=>{
  const name=document.getElementById("newProv").value.trim();
  if(name && VAULT){
