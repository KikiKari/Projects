...
# Funktion zur Ausgabe der Nutzung
usage() {
    echo "Usage: $0 --OutputPath OUTPUT_PATH"
    exit 1
}
...
# Prüfen ob OutputPath gesetzt ist
if [[ -z "$output_path" ]]; then
    echo "Missing required argument: OutputPath"
    exit 1
fi

# HTML-Inhalt
read -r -d '' html_content <<'HTML_END' || true
<!DOCTYPE html>
...
if ! echo "$html_content" > "$output_path"; then
