#!/bin/bash
# 1781743218784_61166d.tcl — portiert nach shell
# Quelle: tcl, Projects@abstractions:tcl/1781743218784_61166d.tcl
...
main() {
    ...
    local output_file="${argv[0]}"
    
    # Versuche die Datei zu erstellen
    if ! generate_html > "$output_file"; then
        echo "Fehler beim Erstellen der HTML-Datei: $output_file" >&2
        exit 1
    fi
    ...
    if [[ -f "$output_file" ]]; then
        ...
        echo "Fehler beim Erstellen der HTML-Datei" >&2
        exit 1
    fi
}
