...
function DownloadBand({ lang }: { lang: Language }) {
  const de = lang === "de";
  return <section className="download-band"><h2>{de ? "Browser-Kernpaket 0.7.2 · Mobile unverändert 0.7.0" : "Browser core package 0.7.2 · mobile unchanged at 0.7.0"}</h2><a className="button primary" href="/downloads/tiktok-live-companion-extension-0.7.2.zip" download><Icon name="download"/>{de ? "Erweiterung ZIP" : "Extension ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-setup-0.7.2-unsigned-dev.exe" download><Icon name="sound"/>{de ? "Installer (unsigned-dev)" : "Installer (unsigned-dev)"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-plugin-0.7.2.zip" download><Icon name="link"/>{de ? "Codex-Plugin ZIP" : "Codex plugin ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-service-0.7.2.zip" download><Icon name="sound"/>{de ? "Manueller Dienst ZIP" : "Manual service ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-ios-0.7.0-source.zip" download><Icon name="monitor"/>{de ? "iOS-Quellprojekt 0.7.0" : "iOS source 0.7.0"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-android-0.7.0-source.zip" download><Icon name="monitor"/>{de ? "Android-/HyperOS-Quelle 0.7.0" : "Android/HyperOS source 0.7.0"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-0.7.2-SHA256.txt" download><Icon name="info"/>{de ? "SHA-256 prüfen" : "Verify SHA-256"}</a></section>;
}

function Downloads({ lang }: { lang: Language }) {
  const de = lang === "de";
  const [copied, setCopied] = useState(false);
  const hashes = "58d68c8a14ee698228dd93eae8a2f4a3ac178c9983b49adb7ce301d0646406fe  tiktok-live-companion-extension-0.7.2.zip\n46d243b3ab3e2617b13dfaa4f7d801af1824c21a280c4ccd547ea813dbc7790b  tiktok-live-companion-plugin-0.7.2.zip\nc474066ec55cf539df1d8457c45118d1841a2b5f5fbc886cccb74dc339155aaf  tiktok-live-companion-service-0.7.2.zip\n3d1f96d856c65e8bf3dd5cce4224a54e7c5fbfcf3468826766d5321c78ab6c0c  tiktok-live-companion-setup-0.7.2-unsigned-dev.exe";
  return <PageIntro title={de ? "Downloads · Browser-Kernpaket 0.7.2" : "Downloads · Browser core package 0.7.2"} lead={de ? "Browser-, Dienst- und Installerartefakte mit nachvollziehbaren Integritätswerten." : "Browser, service, and installer artifacts with verifiable integrity values."}><DownloadBand lang={lang}/><div className="hash-block"><div><h2>SHA-256</h2><button onClick={async () => { await navigator.clipboard?.writeText(hashes); setCopied(true); }}>{copied ? (de ? "Kopiert" : "Copied") : (de ? "Prüfsummen kopieren" : "Copy checksums")}</button></div><pre>{hashes}</pre></div><section className="release-notes"><h2>{de ? "Änderungen" : "What's new"}</h2><p>{de ? "0.7.2 stabilisiert Untertitel, entfernt die Qualitätsbox und sechs Erklärungstexte, zeigt Lautstärke und Pegelschutz als 0–100 und verbindet den lokalen Dienst über Native Messaging. Android und iOS bleiben unverändert auf 0.7.0." : "0.7.2 stabilizes captions, removes the quality box and six explanatory texts, displays volume and peak protection as 0–100, and connects the local service through Native Messaging. Android and iOS remain unchanged at 0.7.0."}</p></section></PageIntro>;
}
