...
function PlatformMatrix({ lang }: { lang: Language }) {
  ...
  return <section className="platform-section page-width"><h2>{de ? "Version 0.7.1 auf drei Plattformen" : "Version 0.7.1 across three platforms"}</h2><p>{de ? "Eine Oberfläche, plattformspezifische und klar ausgewiesene Audiowege." : "One interface with platform-specific, clearly identified audio paths."}</p><div className="platform-grid">{platforms.map(([name, stack, body]) => <article key={name}><Icon name="monitor"/><h3>{name}</h3><strong>{stack}</strong><p>{body}</p></article>)}</div></section>;
}
