...
function PlatformMatrix({ lang }: { lang: Language }) {
  ...
  return <section className="platform-section page-width"><h2>{de ? "Version 0.8.0 auf drei Plattformen" : "Version 0.8.0 across three platforms"}</h2><p>{de ? "Eine Oberfläche, plattformspezifische und klar ausgewiesene Audiowege." : "One interface with platform-specific, clearly identified audio paths."}</p><div className="platform-grid">{platforms.map(([name, stack, body]) => <article key={name}><Icon name="monitor"/><h3>{name}</h3><strong>{stack}</strong><p>{body}</p></article>)}</div></section>;
}
...
function DownloadBand({ lang }: { lang: Language }) {
  const de = lang === "de";
  return <section className="download-band"><h2>{de ? "Browser-Erweiterung, Codex-Plugin, Dienst und Mobile-Quellen 0.8.0" : "Browser extension, Codex plugin, service, and mobile sources 0.8.0"}</h2><a className="button primary" href="/downloads/tiktok-live-companion-extension-0.8.0.zip" download><Icon name="download"/>{de ? "Erweiterung ZIP" : "Extension ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-plugin-0.8.0.zip" download><Icon name="link"/>{de ? "Codex-Plugin ZIP" : "Codex plugin ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-service-0.8.0.zip" download><Icon name="sound"/>{de ? "Windows-Dienst ZIP" : "Windows service ZIP"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-ios-0.8.0-source.zip" download><Icon name="monitor"/>{de ? "iOS-Quellprojekt" : "iOS source project"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-android-0.8.0-source.zip" download><Icon name="monitor"/>{de ? "Android-/HyperOS-Quelle" : "Android/HyperOS source"}</a><a className="button secondary" href="https://github.com/KikiKari/Projects/releases/download/tlc-android-v0.8.0-alpha/tiktok-live-companion-android-0.8.0.apk"><Icon name="download"/>{de ? "Android-Test-APK" : "Android test APK"}</a><a className="button secondary" href="/downloads/tiktok-live-companion-0.8.0-SHA256.txt" download><Icon name="info"/>{de ? "SHA-256 prüfen" : "Verify SHA-256"}</a></section>;
}

function Downloads({ lang }: { lang: Language }) {
  const de = lang === "de";
  const [copied, setCopied] = useState(false);
  const hashes = "9d7701556791b567faf5e1d570dd921f51b8cc5537bed4544da5516f0ed58c00  tiktok-live-companion-extension-0.8.0.zip\n418abe84514fc76e1d1a30b9a9e163100736ec22705d6025742a08335dcf4a2e  tiktok-live-companion-plugin-0.8.0.zip\n0520b3675b4f79bef9c7e0ecc32de3ea16b84e0ca13fa6a030b2d3c53f156674  tiktok-live-companion-service-0.8.0.zip\n1d0549dd0c8cd935653b0ebcbaecf07a6edd1647c1125a0282a836e1c839ca37  tiktok-live-companion-ios-0.8.0-source.zip\n7d3473cbb7e0eec839fd73e79471fd3e00d8040a4a11cbfd2d2a455c9e538363  tiktok-live-companion-android-0.8.0-source.zip\n347bdf26bad4c3f9e6d364587e8aac9d978cb26a0a0da2361c72d81d4d3ab68b  tiktok-live-companion-android-0.8.0.apk";
  return <PageIntro title={de ? "Downloads · Browser-Erweiterung 0.8.0" : "Downloads · Browser extension 0.8.0"} lead={de ? "Browser-, Dienst- und Mobile-Quellartefakte mit nachvollziehbaren Integritätswerten." : "Browser, service, and mobile-source artifacts with verifiable integrity values."}><DownloadBand lang={lang}/><div className="hash-block"><div><h2>SHA-256</h2><button onClick={async () => { await navigator.clipboard?.writeText(hashes); setCopied(true); }}>{copied ? (de ? "Kopiert" : "Copied") : (de ? "Prüfsummen kopieren" : "Copy checksums")}</button></div><pre>{hashes}</pre></div><section className="release-notes"><h2>{de ? "Änderungen" : "What's new"}</h2><p>{de ? "Version 0.8.0 konsolidiert Browser, Android und iOS: tab-isolierte LIVE-Empfehlungen, dauerhafte Sprach- und Chat-Konfiguration, Caption-RAW-/JSON-L-Exporte, vollständige Debug-Rohereignisse sowie ein einstellbares Auto-Reconnect. Die bestehenden mobilen VLC-Schaltflächen, Wiedergabepfade und Installationswege bleiben unverändert." : "Version 0.8.0 consolidates browser, Android, and iOS with tab-isolated LIVE recommendations, persistent speech and chat settings, caption RAW/JSON-L exports, complete raw debug events, and configurable auto-reconnect. Existing mobile VLC buttons, playback paths, and installation flows remain unchanged."}</p></section></PageIntro>;
}
