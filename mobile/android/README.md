# TikTok LIVE Companion Android/HyperOS 0.8.0
...
- `mockDebug`: reproducible UI/bridge fallback build that reports **ShazamKit nicht konfiguriert**, requires no proprietary SDK, and uses the main application ID for local 0.8.0 device installs.
...
The packaging script renames the verified mock debug output to `tiktok-live-companion-android-0.8.0.apk`. A real catalog build additionally requires the official AAR and configured token service.
