#!/usr/bin/env bash
# git-publish-gateway.sh
#
# Wird vom OpenClaw Cron-Job 'git-publish-gateway' aufgerufen.
# Pusht lokale Workspace-Änderungen auf den jeweiligen Gateway-Branch.
#
# Exit-Codes:
...
#   4 = Workspace-Pfad nicht erreichbar
#
# Output: maschinen- UND menschen-lesbar in eine Zeile pro Event,
# Format: STATUS=<status> KEY=value KEY=value ...

set -u  # nicht set -e, wir wollen Errors kontrolliert behandeln

WORKSPACE="/home/openclaw/.openclaw/workspace"
LOG_DIR="/home/openclaw/.openclaw/logs/git-publish-gateway"
...
# --- Schritt 1: Workspace betretbar? ---
if ! cd "$WORKSPACE" 2>/dev/null; then
  log "STATUS=error CODE=4 REASON=workspace-unreachable PATH=$WORKSPACE"
  exit 4
fi

# --- Schritt 2: Branch ermitteln ---
BRANCH=$(git branch --show-current 2>/dev/null)
if [[ "$BRANCH" != "gateway1" && "$BRANCH" != "gateway2" ]]; then
...
while IFS= read -r line; do
  # Format: " M path/to/file" oder "?? path/to/file"
  # Wir wollen Spalte 2+ extrahieren (Pfad), egal welche Status-Markierung
  file_path="${line:3}"

  # Skip wenn es kein File ist (z.B. gelöschtes File - existiert ja nicht mehr zum Scannen)
  if [[ -f "$file_path" ]]; then
    if grep -E -l "$SECRET_PATTERNS" "$file_path" 2>/dev/null > /dev/null; then
      # Welches Muster wurde gefunden? (für Reporting, NICHT den Key zeigen)
...
COMMIT_MSG="auto: gateway-sync $(date '+%Y-%m-%d %H:%M')"
if ! git commit -m "$COMMIT_MSG" 2>&1 | tee -a "$LOG_FILE"; then
  # Spezialfall: wenn nach git add -A nichts gestagt wurde (alles in .gitignore)
